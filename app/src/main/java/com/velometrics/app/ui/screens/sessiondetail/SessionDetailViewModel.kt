...
class SessionDetailViewModel @Inject constructor(
    ...
    /**
     * Deletes the currently loaded session (#193) via [RideLifecycle.delete] — FK cascades remove
     * its `interval_sessions`/`session_best_efforts` rows. On success navigates back; on failure
     * surfaces [deleteError].
     */
    fun deleteRide() {
        val current = _session.value ?: return
        viewModelScope.launch {
            when (rideLifecycle.delete(listOf(current.id))) {
                DeleteResult.Deleted -> _navigateBack.emit(Unit)
                DeleteResult.Failed -> _deleteError.value = "Couldn't delete ride"
            }
        }
    }
