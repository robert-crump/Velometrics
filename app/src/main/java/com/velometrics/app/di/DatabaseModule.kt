...
object DatabaseModule {
    ...
    // avgPower30sAfter (user follow-up to #178): mirrors avgPower60sAfter but over the 30s window,
    // same nullable-column/no-backfill shape as MIGRATION_15_16.
    internal val MIGRATION_16_17 = object : Migration(16, 17) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE interval_sessions ADD COLUMN avgPower30sAfter INTEGER")
        }
    }
    ...
    // HR / elevation vs. distance series (#204): nullable JSON column, populated at import only
    // (no backfill -- the FIT data it derives from isn't retained).
    internal val MIGRATION_20_21 = object : Migration(20, 21) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE cycling_sessions ADD COLUMN hrDistanceSeries TEXT")
        }
    }

    // Peak power / heart rate / speed for the Ride Detail stat lines: nullable, import-time only.
    internal val MIGRATION_21_22 = object : Migration(21, 22) {
        override fun migrate(database: SupportSQLiteDatabase) {
            database.execSQL("ALTER TABLE cycling_sessions ADD COLUMN maxPower INTEGER")
            database.execSQL("ALTER TABLE cycling_sessions ADD COLUMN maxHeartRate INTEGER")
            database.execSQL("ALTER TABLE cycling_sessions ADD COLUMN maxSpeedKmh REAL")
        }
    }
